    ...
    return (
        brandList.map(item => (
            <tr key={item.id}>
                ...
                <td className="text-center">
                    <Link to={`edit-brand/${item.id}`} className="btn2 btn2-primary2 btn2-lg mr-3" style={{ width: '70px' }}>Edit</Link>
                    <button type="button" onClick={(e) => deleteBrand(e, item.id)} className="btn2 btn2-danger2 btn2-lg" style={{ width: '70px' }}>Delete</button>
                </td>
            </tr>
        ))
    )
