    ...
    return (
        sliderList.map(slider => (
            <tr key={slider.id}>
                ...
                <td className="text-center">
                    <Link to={`edit-slider/${slider.id}`} className="btn2 btn2-primary2 btn2-lg mr-3" style={{ width: '70px' }}>Sửa</Link>
                    <button type="button" onClick={(e) => deleteSlider(e, slider.id)} className="btn2 btn2-danger2 btn2-lg" style={{ width: '70px' }}>Xoá</button>
                </td>
            </tr>
        ))
    )
